...
# Configuration
# Use paths relative to the project root or package
BASE_DIR = Path(__file__).resolve().parent.parent.parent
MODEL_PATH = BASE_DIR / 'models' / 'ocr_model_final.pth'
# Attempt to find mapping file, default to a sensible location or handle missing
MAPPING_FILE = BASE_DIR / 'bhasha' / 'data' / 'grapheme_maps.json' 
...
if MAPPING_FILE.exists():
    try:
        ...
        print(f"✅ Loaded grapheme maps from {MAPPING_FILE}")
    except Exception as e:
        print(f"⚠️ Error loading grapheme maps: {e}")
else:
    print(f"⚠️ Grapheme maps not found at {MAPPING_FILE}. OCR decoding will result in '?'")
...
try:
    ...
    if MODEL_PATH.exists():
        ...
        print(f"⚠️ Model not found at {MODEL_PATH}. Using untrained model (random weights).")
    ocr_model.eval()
except Exception as e:
