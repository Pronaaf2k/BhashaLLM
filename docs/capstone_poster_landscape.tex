...
\vspace{0.2cm}
\begin{itemize}
  \item OCR: বাংলা হস্তলিখন ডেটাসেট (গ্রাফিম-রুট, ভাওয়েল, কনসোন্যান্ট)।
...
\begin{itemize}
  ...
  \item API-চালিত টিউটর ও ফিলসফিক্যাল রেসপন্স।
...
\begin{tikzpicture}[
  ...
  \node[box, below=of pre] (ocr) {OCR মডেল \\ (ResNet + Grapheme)};
  \node[box, below=of ocr] (text) {রিকগনাইজড টেক্সট};
  \node[box, below=of text] (llm) {LLM টিউটর/ফিডব্যাক};
  \node[box, below=of llm] (api) {FastAPI রেসপন্স};
...
\begin{itemize}
  ...
  \item \textbf{/api/philosophical} — সাহিত্যিক প্রতিফলন
\end{itemize}
\vspace{0.2cm}
...
\vspace{0.2cm}
\begin{itemize}
  \item \textbf{Email:} yourname@university.edu
